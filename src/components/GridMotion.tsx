...
const GridMotion = ({ items = [], gradientColor = 'black' }: GridMotionProps) => {
  ...
  useEffect(() => {
    ...
    const updateMotion = () => {
      ...
      const offset = (gridCenter - windowCenter) / window.innerHeight;

      rowRefs.current.forEach((row, index) => {
        if (row) {
          const direction = index % 2 === 0 ? 1 : -1;
          const moveAmount = offset * maxMoveAmount * direction;
              ...
              {[...Array(7)].map((_, itemIndex) => {
                ...
                return (
                  <div key={itemIndex} className="row__item">
                    <div className="row__item-inner bg-card border border-border shadow-md text-foreground font-medium">
